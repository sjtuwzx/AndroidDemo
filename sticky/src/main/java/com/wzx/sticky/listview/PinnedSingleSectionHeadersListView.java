...
public class PinnedSingleSectionHeadersListView extends ListView implements View.OnClickListener {
    ...
    public interface PinnedSingleSectionedHeaderAdapter {
        ...
        View getPinnedHeaderView(int position, View convertView,
                                        ViewGroup parent);
        int getHeaderCount();

        void setOnDataChangeListener(SingleSectionBaseAdapter.OnDataChangeListener listener);
    ...
    private void ensurePinnedHeaderLayout(View header, boolean focus) {
        if (focus || header.isLayoutRequested()) {
            int widthSpec = MeasureSpec.makeMeasureSpec(getWidth() - getPaddingLeft() - getPaddingRight(),
                    mWidthMode);
